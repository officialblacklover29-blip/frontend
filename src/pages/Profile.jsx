...
const Profile = ({ darkMode }) => {
  ...
  const [activeTab, setActiveTab] = useState("posts"); // UI State for Tabs
  ...
  // --- STATIC DATA FOR DESIGN (Highlights & Dummy Posts) ---
  const highlights = [
  ...
  // Generating dummy posts based on user.posts count (Visual Only)
  const displayPosts = Array.from({ length: user.posts || 6 }).map((_, i) => ({
      id: i, img: `https://loremflickr.com/400/400/india,vibe?random=${i}`
  }));
  ...
  const styles = {
    ...
    username: { fontSize: "28px", fontWeight: "300", margin: 0, fontFamily: "'Playfair Display', serif" },
    ...
    // Stats
    statsRow: { display: "flex", gap: "40px", marginBottom: "20px", fontSize: "16px" },
    statNumber: { fontWeight: "bold", fontSize: "18px" },

    // Bio
    bioSection: { fontSize: "15px", lineHeight: "1.6", maxWidth: "500px" },
    fullName: { fontWeight: "bold", display: "block", marginBottom: "4px", fontSize: "16px" },
    ...
    gridItem: { position: "relative", paddingBottom: "100%", backgroundColor: theme.bg, cursor: "pointer", overflow: "hidden", borderRadius: "4px" },
             ...
             {!isEditing ? (
               <>
                 ...
                 <div style={styles.statsRow}>
                     <span><span style={styles.statNumber}>{user.posts || 0}</span> posts</span>
               ...
               <div style={{maxWidth: '400px'}}>
                 <h3 style={{marginBottom: '15px', fontFamily: "'Playfair Display', serif"}}>Edit Profile</h3>
                 ...
                 <div style={{marginTop: '10px', marginBottom: '20px'}}>
                    ...
                    <input type="file" onChange={(e) => setFormData({...formData, file: e.target.files[0]})} style={{fontSize: '13px'}} />
                 </div>
      ...
      {activeTab === 'posts' && (
          <div style={styles.grid}>
             ...
             {displayPosts.length === 0 && (
                <div style={{gridColumn: "1 / -1", ...styles.emptyState}}>No moments shared yet 📸</div>
             )}
          </div>
      )}
